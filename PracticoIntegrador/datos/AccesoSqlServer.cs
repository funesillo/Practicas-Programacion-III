...
public class AccesoSqlServer : IAccesoDatos
{
    ...
    private readonly string _csApp = "Server=localhost,1433;User Id=sa;Password=Curso.NET2026;Database=practico;TrustServerCertificate=True";

    public AccesoSqlServer(string connectionString) => _csMaster = connectionString;
    ...
    public void CrearEstructura()
    {
        ...
        {
            conn.Open();
            using var cmd = new SqlCommand("IF DB_ID('practico') IS NULL CREATE DATABASE practico;", conn);
            cmd.ExecuteNonQuery();
        }

        using (var conn = new SqlConnection(_csApp))
        {
            conn.Open();
            string sql = @"
                ...
                CREATE TABLE categorias (id INT IDENTITY PRIMARY KEY, nombre VARCHAR(100) NOT NULL);
                CREATE TABLE clientes (id INT IDENTITY PRIMARY KEY, nombre VARCHAR(100) NOT NULL, email VARCHAR(100) NOT NULL UNIQUE);
                CREATE TABLE productos (id INT IDENTITY PRIMARY KEY, nombre VARCHAR(100) NOT NULL, precio DECIMAL(18,2) NOT NULL, stock INT NOT NULL, categoria_id INT FOREIGN KEY REFERENCES categorias(id));
                CREATE TABLE pedidos (id INT IDENTITY PRIMARY KEY, cliente_id INT FOREIGN KEY REFERENCES clientes(id), fecha DATETIME DEFAULT CURRENT_TIMESTAMP);
                CREATE TABLE detalle_pedido (pedido_id INT FOREIGN KEY REFERENCES pedidos(id), producto_id INT FOREIGN KEY REFERENCES productos(id), cantidad INT NOT NULL, precio_unit DECIMAL(18,2) NOT NULL, PRIMARY KEY (pedido_id, producto_id));";
    ...
    public void EjecutarOperaciones()
    {
        ...
        using (var cmd = new SqlCommand("SELECT p.id, p.nombre, p.precio, c.nombre as cat FROM productos p JOIN categorias c ON p.categoria_id = c.id ORDER BY p.id;", conn))
        ...
        using (var tx = conn.BeginTransaction())
        {
            try
            {
                var cmdPed = new SqlCommand("INSERT INTO pedidos (cliente_id) VALUES (@cId); SELECT CAST(SCOPE_IDENTITY() AS INT);", conn, tx);
                cmdPed.Parameters.AddWithValue("@cId", 1);
                int pId = (int)cmdPed.ExecuteScalar();

                var cmdDet = new SqlCommand("INSERT INTO detalle_pedido (pedido_id, producto_id, cantidad, precio_unit) VALUES (@pId, @prodId, @cant, @precio);", conn, tx);
        ...
        using (var cmd = new SqlCommand("UPDATE productos SET precio = precio * 1.10 WHERE categoria_id = 1;", conn))
            Console.WriteLine($"\n[U1] Subí 10% precios de categoría #1 -> {cmd.ExecuteNonQuery()} filas.");

        using (var cmd = new SqlCommand("DELETE FROM detalle_pedido WHERE pedido_id = 1 AND producto_id = 2;", conn))
    ...
    public void DemostrarRollback()
    {
        ...
        try
        {
            using var cmd = new SqlCommand("UPDATE productos SET precio = 1.00 WHERE id = 1;", conn, tx);
            cmd.ExecuteNonQuery();
