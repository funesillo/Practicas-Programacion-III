...
public class AccesoMySql : IAccesoDatos
{
    ...
    private readonly string _csApp = "Server=localhost;Port=3306;User=root;Password=Curso.NET2026;Database=practico;";

    public AccesoMySql(string connectionString) => _csMaster = connectionString;

    public void CrearEstructura()
    {
        using (var conn = new MySqlConnection(_csMaster))
        {
            conn.Open();
        ...
        using (var conn = new MySqlConnection(_csApp))
        {
            conn.Open();
            string sql = @"
                SET FOREIGN_KEY_CHECKS = 0;
                DROP TABLE IF EXISTS detalle_pedido, pedidos, productos, clientes, categorias;
                SET FOREIGN_KEY_CHECKS = 1;
                CREATE TABLE categorias (id INT AUTO_INCREMENT PRIMARY KEY, nombre VARCHAR(100) NOT NULL);
                CREATE TABLE clientes (id INT AUTO_INCREMENT PRIMARY KEY, nombre VARCHAR(100) NOT NULL, email VARCHAR(100) NOT NULL UNIQUE);
                CREATE TABLE productos (id INT AUTO_INCREMENT PRIMARY KEY, nombre VARCHAR(100) NOT NULL, precio DECIMAL(18,2) NOT NULL, stock INT NOT NULL, categoria_id INT, FOREIGN KEY (categoria_id) REFERENCES categorias(id));
                CREATE TABLE pedidos (id INT AUTO_INCREMENT PRIMARY KEY, cliente_id INT, fecha DATETIME DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (cliente_id) REFERENCES clientes(id));
                CREATE TABLE detalle_pedido (pedido_id INT, producto_id INT, cantidad INT NOT NULL, precio_unit DECIMAL(18,2) NOT NULL, PRIMARY KEY (pedido_id, producto_id), FOREIGN KEY (pedido_id) REFERENCES pedidos(id), FOREIGN KEY (producto_id) REFERENCES productos(id));";
    ...
    public void EjecutarOperaciones()
    {
        ...
        using (var cmd = new MySqlCommand("SELECT p.id, p.nombre, p.precio, c.nombre as cat FROM productos p JOIN categorias c ON p.categoria_id = c.id ORDER BY p.id;", conn))
        ...
        using (var tx = conn.BeginTransaction())
        {
            try
            {
                var cmdPed = new MySqlCommand("INSERT INTO pedidos (cliente_id) VALUES (@cId); SELECT LAST_INSERT_ID();", conn, tx);
                cmdPed.Parameters.AddWithValue("@cId", 1);
                int pId = Convert.ToInt32(cmdPed.ExecuteScalar());

                var cmdDet = new MySqlCommand("INSERT INTO detalle_pedido (pedido_id, producto_id, cantidad, precio_unit) VALUES (@pId, @prodId, @cant, @precio);", conn, tx);
        ...
        using (var cmd = new MySqlCommand("UPDATE productos SET precio = precio * 1.10 WHERE categoria_id = 1;", conn))
            Console.WriteLine($"\n[U1] Subí 10% precios de categoría #1 -> {cmd.ExecuteNonQuery()} filas.");

        using (var cmd = new MySqlCommand("DELETE FROM detalle_pedido WHERE pedido_id = 1 AND producto_id = 2;", conn))
    ...
    public void DemostrarRollback()
    {
        ...
        try
        {
            using var cmd = new MySqlCommand("UPDATE productos SET precio = 1.00 WHERE id = 1;", conn, tx);
            cmd.ExecuteNonQuery();
